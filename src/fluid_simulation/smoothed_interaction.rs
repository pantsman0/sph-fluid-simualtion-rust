...
impl SmoothedInteraction {
    ...
    pub fn calculate_acceleration_due_to_pressure(
        &self,
        particle: &Particle,
        ...
        {
            if particle.id == iter_particle.id {
                continue;
            }
            ...
            let shared_pressure =
                self.calculate_shared_pressure(particle.local_density, iter_particle.local_density);
            property_gradient +=
                relative_position.normalise() * shared_pressure * slope * iter_particle.mass
                    / iter_particle.local_density;
        }
    ...
    pub fn calculate_viscosity(
            ...
            .map(|index| &particles[index])
        {
            ...
            viscosit_force += relative_speed * self.viscosity * iter_particle.mass * influence
                / iter_particle.local_density;
        }
